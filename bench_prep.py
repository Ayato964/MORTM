...
DATA_DIR = "data"
CACHE = "bench/cache"
BLOCK_M = 8   # PAST/CONST/FUTURE 各8小節 → 計24小節(長文脈)
PROGRAM = ["PIANO", "SAX"]
SEED = 42


class ContextDataMaker(FoundationDataMaker):
    """長文脈ブロックを貪欲充填で生成。PAST→CONST→FUTURE の順に最大 block_max 小節ずつ詰める。
    形式: <EOS> [<PAST_M>..<TAG_END>] [<CONST_M>..<TAG_END>] [<FUTURE_M>..<TAG_END>] <TE>
    - 24小節以上: 8/8/8 のフル窓を非重複スライド(Moonbeam eval と同じ)。
    - 24未満: 例)12→PAST8/CONST4/FUTURE0、5→PAST5のみ。空ブロックは省略。短い曲も残す。
    """
    def convert_greedy_context(self, block_max=8):
        ...
        while now < max_meas:
            ...
            now += min(block_max * 3, avail_global)   # 非重複スライド


def midi_to_blocks(tok, ids, midi_dir, midi_file):
    """1 MIDI -> 長文脈ブロックのリスト。失敗時 []。
    形式: <EOS> [<PAST_M>..<TAG_END>][<CONST_M>..][<FUTURE_M>..] <TE> (PAST貪欲充填, 最大24小節)。
    """
    try:
        ...
        for a in m.aya_node:
            sl = np.asarray(a)
            if sl.ndim == 0 or len(sl) <= 4:   # aya_node[0]=0 のダミー等を除外
                continue
...
def build(dataset, items, label_names, tok, ids, tag=""):
    """items: list of (split, midi_dir, midi_file, label_idx) -> split別に変換キャッシュ。
    tag: split_seed違いのキャッシュを分けるための接尾辞(例 '_s1337')。"""
    ...
    for j, (split, d, f, lab) in enumerate(items, 1):
        ...
        if not blocks:
            ...
            for b in blocks:
                ...
                by_split[split]["file_ids"].append(j)   # 同一MIDI由来クリップは同じID(file単位集約用)
        if j % 200 == 0:
...
def prep_emopia(tok, ids, split_seed=42, tag=""):
    """感情4クラス。公式split(train_clip/val_clip/test_clip)。label=ファイル名Q1-4。"""
...
def _stratified_split(by_label, seed=SEED, ratios=(0.8, 0.1, 0.1)):
    """label -> [files] を層化して split 割当。戻り: list of (split, file, label_idx)。"""
...
def prep_pianist8(tok, ids, split_seed=42, tag=""):
    """作曲家8クラス。midi/{composer}/*.mid。層化split。"""
...
def prep_midikong(tok, ids, split_seed=42, tag="", topk=30):
    """奏者(player)分類。曲数上位topk奏者。Moonbeam PiJAMA30 相当(30奏者)。層化split。"""
    zp = os.path.join(DATA_DIR, "midi_kong.zip")
    tmp = tempfile.mkdtemp(prefix="midikong_")
    with zipfile.ZipFile(zp) as z:
        z.extractall(tmp)
    # midi_kong/{live,studio}/{performer}/**/*.midi -> performer別に収集
    perf_files = {}
